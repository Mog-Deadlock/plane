...
// components
import { GithubOAuthButton, GitlabOAuthButton, GoogleOAuthButton, MogOAuthButton } from "@/components/account";
// hooks
...
export const OAuthOptions: React.FC<TOAuthOptionProps> = observer(() => {
  ...
  const isOAuthEnabled =
    (config && (config?.is_google_enabled || config?.is_mog_enabled || config?.is_github_enabled || config?.is_gitlab_enabled)) ||
    false;
  ...
  return (
    <>
      ...
      <div className={`mt-7 grid gap-4 overflow-hidden`}>
        ...
        {config?.is_mog_enabled && <MogOAuthButton text="Continue with MOG" />}
